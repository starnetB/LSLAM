...
bool Dataset::Init(){
    //read camera intrisics and extriniscs
    ...
    for(int i=0;i<4;++i){
        ...
        Vec3 t;
        t<<projection_data[3],projection_data[7],projection_data[9];
        t=K.inverse()*t;  //t [-f_u b,0,0]  [-b,0,0] ,b应该 想对于 stero camera 的为宜 stero->single -b 就是single->ster0
        //后面他按一定比列缩放了0.5,所以内参要跟着缩放
        K=K*0.5;
        // t.norm就是基线
